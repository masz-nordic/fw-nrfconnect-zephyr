...
struct flash_mspi_nor_config {
	...
	struct mspi_dev_cfg initial_cfg;
...
static int acquire(const struct device *dev, const struct mspi_dev_cfg * cfg)
{
	const struct flash_mspi_nor_config *dev_config = dev->config;
		    ...
		    size_t size)
{
	...
	rc = acquire(dev, &dev_config->mspi_cfg);
	if (rc < 0) {
		return rc;
	}
...
static int status_get(const struct device *dev, const struct mspi_dev_cfg * curr_cfg, uint8_t *status)
{
	const struct flash_mspi_nor_config *dev_config = dev->config;
	struct flash_mspi_nor_data *dev_data = dev->data;
	int rc;

	if (curr_cfg != &dev_config->initial_cfg) {
		rc = dev_cfg_apply(dev, &dev_config->initial_cfg);

		if(rc < 0) {
			LOG_ERR("Switching to initial dev_cfg failed: %d", rc);
			return rc;
		}
	}
	...
	if (curr_cfg != &dev_config->initial_cfg) {
		rc = dev_cfg_apply(dev, &dev_config->mspi_cfg);

		if(rc < 0) {
			LOG_ERR("Switching to default dev_cfg failed: %d", rc);
		}
	}
...
static int wait_until_ready(const struct device *dev, const struct mspi_dev_cfg * curr_cfg, k_timeout_t poll_period)
{
	int rc;
	uint8_t status_reg;

	while (true) {
		rc = status_get(dev, curr_cfg, &status_reg);
		     ...
		     size_t size)
{
	...
	while (size > 0) {
		...
		rc = wait_until_ready(dev, &dev_config->mspi_cfg, K_MSEC(1));
		if (rc < 0) {
			break;
		}
	}
...
static int api_erase(const struct device *dev, off_t addr, size_t size)
{
	...
	rc = acquire(dev, &dev_config->initial_cfg);
	if (rc < 0) {
		return rc;
	}
	...
	while (size > 0) {
		...
		rc = wait_until_ready(dev, &dev_config->initial_cfg, K_MSEC(1));
		if (rc < 0) {
			break;
		}
	}
			 ...
			 size_t size)
{
	...
	rc = acquire(dev, dev_config->jedec_cmds->sfdp.force_single ?
			  &dev_config->initial_cfg : &dev_config->mspi_cfg);
	if (rc < 0) {
		return rc;
	}
...
{
	int rc = acquire(dev, dev_config->jedec_cmds->id.force_single ?
			      &dev_config->initial_cfg : &dev_config->mspi_cfg);
	if (rc < 0) {
		return rc;
	}
...
static int switch_mode(const struct device *dev, uint8_t *id)
{
	 ...
	 */
	uint8_t hp_payload[3] = {SPI_NOR_QE_BIT, 0x0, FLASH_MX25R_LH_MASK};
	if (((dev_config->mspi_cfg.io_mode == MSPI_IO_MODE_QUAD_1_4_4) &&
	     (dev_config->mspi_cfg.freq > MHZ(8))) ||
	    ((dev_config->mspi_cfg.io_mode == MSPI_IO_MODE_SINGLE) &&
	     (dev_config->mspi_cfg.freq > MHZ(33)))) {
		dev_data->packet.data_buf  = hp_payload;
		dev_data->packet.num_bytes = sizeof(hp_payload);
	} else {
...
static int flash_chip_init(const struct device *dev)
{
	...
	rc = mspi_dev_config(dev_config->bus, &dev_config->mspi_id,
			     MSPI_DEVICE_CONFIG_ALL, &dev_config->initial_cfg);
	if (rc < 0) {
	...
	rc = mspi_dev_config(dev_config->bus, &dev_config->mspi_id,
			     ...
			     dev_config->jedec_cmds->id.force_single ?
			     &dev_config->initial_cfg : &dev_config->mspi_cfg);
	if (rc < 0) {
...
#define FLASH_MSPI_NOR_INST(inst)						\
	...
	static const struct flash_mspi_nor_config dev##inst##_config = {	\
		...
		.initial_cfg = FLASH_INITIAL_CONFIG(inst),			\
